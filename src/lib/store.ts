...
export function calcBooking(input: BookingCalcInput, data: DB = db) {
  ...
  const rate = perNight > data.settings.gstThreshold ? data.settings.gstSlabHigh : data.settings.gstSlabLow;
  const tax = (taxable * rate) / 100;
  const igst = input.interState ? tax : 0;
  const cgst = input.interState ? 0 : tax / 2;
  const sgst = input.interState ? 0 : tax / 2;
  return {
    roomCost, mealCost, extraBedCost, extraAdult, subtotal, discount, taxable,
    taxRate: rate, cgst, sgst, igst, tax, total: taxable + tax,
  };
}
...
export const bookingService = {
  create(payload: Partial<Booking> & { guestId: ID }): Booking {
    ...
    update((d) => {
      const b: Booking = {
        id: nextNo("booking", "MYR-B"),
        grc: nextNo("grc", d.settings.grcPrefix),
        invoiceNo: nextNo("invoice", d.settings.invoicePrefix),
        guestId: payload.guestId,
        source: payload.source ?? "Direct Website",
        roomTypeId: payload.roomTypeId!, roomIds: payload.roomIds ?? [],
      ...
      const calc = calcBooking(
        { roomTypeId: b.roomTypeId, ratePlanId: b.ratePlanId, rateNight: b.rateNight, nights, rooms: b.roomIds.length || 1, adults: b.adults, children: b.children, extraBed: b.extraBed, discount: b.discount },
        d,
      );
      ...
      b.charges.push({
        ...
        description: `${rt?.name} · ${rp?.code} · ${nights} night(s)`,
        qty: nights, rate: (calc.roomCost + calc.mealCost + calc.extraBedCost) / nights,
        amount: calc.roomCost + calc.mealCost + calc.extraBedCost, taxRate: calc.taxRate,
      });
      if (b.discount > 0)
        b.charges.push({ id: uid("ch"), date: b.checkIn, kind: "Discount", description: "Booking discount", qty: 1, rate: b.discount, amount: b.discount, taxRate: 0 });
